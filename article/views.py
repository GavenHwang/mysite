    ...
    if request.method == "POST":
        ...
        if tag_post_form.is_valid():
            try:
                ...
                return HttpResponse("Sorry, The data cannot be save.")
        else:
    ...
    if request.method == "POST":
        ...
        if article_post_form.is_valid():
            ...
            try:
                ...
                new_article.save()
                tags = request.POST['tags']     # 此处的tags是字符串['','']形式的
                if tags:
                    for selected_tag in json.loads(tags):   # json.loads将字符串['','']转为真正的数组
                        tag = request.user.tag.get(tag=selected_tag)
...
def redit_article(request, article_id):
    if request.method == "GET":
        ...
        try:
            ...
            tags = request.POST['tags']  # 此处的tags是字符串['','']形式的
            if tags:
                redit_article.article_tag.set(ArticleTag.objects.filter(tag__in=json.loads(tags)))
            redit_article.save()
            return HttpResponse("1")
        except:
            return HttpResponse("2")
