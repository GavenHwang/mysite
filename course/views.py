...
class CourseListView(ListView):
    ...
    # queryset = Course.objects.filter(user=User.objects.filter(username="GavenHwang")[0])
    template_name = "course/course_list.html"

    def get_queryset(self):
        qs = super(CourseListView, self).get_queryset()
        return qs.filter(user=User.objects.filter(username="GavenHwang")[0])
...
# 前台查看课程详情
class LessonDetailView(LoginRequiredMixin, TemplateResponseMixin, View):
    login_url = "/account/login/"
